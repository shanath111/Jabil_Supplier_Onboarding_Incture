jQuery.sap.declare("oneapp.incture.workbox.util.formatter");
oneapp.incture.workbox.util.formatter = {
	...
	wbInboxTitleCount: function (currentViewPageName, inboxType, count) {
	...
	// to show and sort the task descriptions in the action comment popup.	
	wbCommentsFragSort: function (actionType, oSelectedTaskArray) {
		var visible = false;
		var oAppModel = this._getBindingContext("oAppModel").getModel();
		var oPreviousIndex = parseInt(this._getBindingContext("oAppModel").sPath.split("/")[2], 10) - 1;
		if (oPreviousIndex >= 0) var prevActionType = oSelectedTaskArray[oPreviousIndex].actionType;
		else var prevActionType = null;
		if ((!prevActionType) || (prevActionType !== actionType)) {
			visible = true;
		}
	...
	/*****************ACTION BUTTON - START *********************/
	//setting the visibility of action buttons - sub header action buttons (app view)
    ...
    // Jabil - Start
    wbShowHeader: function(currentView){
        var visible = true;
        if (currentView === "unifiedInbox"){
            visible = true;
            this.addStyleClass("wbTntHeaderStyle");
        } if (currentView === "taskDetailPage"){
            visible = false;
            this.addStyleClass("wbTntHeaderRemove");
        }
        return visible;
    }
    // Jabil - End
