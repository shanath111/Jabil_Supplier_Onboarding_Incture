	...
	var oCustomNode = Node.extend("sap.suite.ui.commons.sample.NetworkGraphCustomRendering.NetworkGraphCustomNode", {
		...
		calculateSizes: function () {
			...
			this.oCustomTrimmedTextSize = this.computeTextDimensions({
				text: this.getModel("oDefaultDataModel").getProperty(this.getBindingContext("oDefaultDataModel").sPath).description,
				width: this.getWidth() - 10,
				attributes: {
					"font-size": "10px"
				},
			...
			if (this.getShape() === "Box") {
				...
				this.setSize({
					...
					height: Math.floor(iTitleHeight + this.oCustomTrimmedTextSize.lineHeight + 10 + 50 /*icon*/)
				});
			}
