(function (window, undefined) {
    ...
    Vend.protocol = "https:";
    Vend.host_trial = "trial.serviceobjects.com";
    Vend.host_live = "ws.serviceobjects.com";

    Vend.endpoint = "AC";
    Vend.limit = 2000;


    Vend.synonyms = Vend.synonyms || [
        { r: /\bN(?=\s)/, w: "NORTH" },
    ...
    Vend.diacritics = Vend.diacritics || [
        { r: /[ÀÁÂÃ]/gi, w: "A" },
        { r: /Å/gi, w: "AA" },
        { r: /[ÆæÄ]/gi, w: "AE" },
        { r: /Ç/gi, w: "C" },
    ...
    Vend.requestQueue = Vend.requestQueue || [];
    Vend.requestCache = Vend.requestCache || {};
    Vend.scriptRequests = Vend.scriptRequests || [];
    Vend.waitingRequest = Vend.waitingRequest || false;
    Vend.blockRequests = Vend.blockRequests || false;


    Vend.styleFixes = Vend.styleFixes || [];
    Vend.agent = Vend.agent || (window.navigator && window.navigator.userAgent) || "";

    Vend.galaxyFix = Vend.galaxyFix || ((/Safari\/534.30/).test(Vend.agent) && (/GT-I8190|GT-I9100|GT-I9305|GT-P3110/).test(Vend.agent));


    Vend.container = Vend.container || null;

    Vend.XMLHttpRequest = Vend.XMLHttpRequest || window.XMLHttpRequest;

    var ready = false,
        readyList = [];

    Vend.fuzzyMatch = typeof Vend.fuzzyMatch === "undefined" ? true : Vend.fuzzyMatch;

    Vend.fuzzyTags = Vend.fuzzyTags || ["*"];


    Vend.ready = Vend.ready || function (delegate) {
        if (ready) {
            if (readyList.length) {
    ...
    function documentLoaded() {
        if (document.addEventListener) {
            Vend.ignore(document, "DOMContentLoaded", documentLoaded);
            ready = true;
            Vend.ready();
        }
        else if (document.readyState === "complete") {
            Vend.ignore(document, "onreadystatechange", documentLoaded);
            ready = true;
            Vend.ready();
        }
    }
    ...
    function checkDocumentLoad() {
        if (document.readyState === "complete") {
            ...
            if (document.addEventListener) Vend.listen(document, "DOMContentLoaded", documentLoaded);
            else Vend.listen(document, "onreadystatechange", documentLoaded);
            Vend.listen(window, "load", documentLoaded);
        }
    }


    Vend.Eventable = Vend.Eventable || function (source) {
    ...
    function postRequestXHR(request) {
        var xhr = new Vend.XMLHttpRequest();

        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200)
                request.callback(Vend.parseJSON(xhr.responseText));
        }
    ...
    function postRequestForm(request) {
        ...
        form.method = "POST";
        form.action = Vend.protocol + "//" + options.Host + "/" + request.service + "/json.ws";
        ...
        iframe.onload = function () {
            if (!loaded) {
                ...
                request.callback({ Items: Vend.parseJSON(iframe.contentWindow.name) });
    ...
    function getRequestXHR(request) {
        ...
        if (request.url.length > Vend.limit) {
            request.post = true;
            postRequest(request);
        }
        else {
    ...
    function getRequestScript(request) {
        var script = Vend.create("script", { type: "text/javascript", async: "async" }),
            head = document.getElementsByTagName("head")[0];

        request.position = Vend.scriptRequests.push(request);
        ...
        if (script.src.length > Vend.limit) {
            request.post = true;
            postRequest(request);
        }
        else
    ...
    function processRequest(request) {
        ...
        if (Vend.blockRequests && Vend.waitingRequest) {
            Vend.requestQueue = [request];
            return;
        }

        if (request.block)
            Vend.blockRequests = true;

        if (request.queue && Vend.waitingRequest) {
            Vend.requestQueue.push(request);
            return;
        }

        Vend.waitingRequest = true;

        if (request.cache && Vend.requestCache[request.url]) {
            function ayncCallback() {
                request.callback(Vend.requestCache[request.url].response);
            }
    ...
    function processResponse(request) {
        Vend.waitingRequest = false;

        if (request.block)
            Vend.blockRequests = false;
        ...
        {
            if (request.service == "Select" || request.service == "SelectValidate")
            {
                if (request.response.Error !== undefined && request.response.Error !== null)
                { 
                    request.error(request.response.Error.Description, request);
                }
                else
                {
                    request.success(request.response.AddressInfos, request.response, request);
                }
            }
            else if (request.service == "Country")
            {
                if (request.response.Error !== undefined && request.response.Error !== null) {
                    request.error(request.response.Error.Description, request);
                }
            ...
            else if (request.service == "PredictPlace" || request.service == "SelectPlace") {
                if (request.response.Error !== undefined && request.response.Error !== null) {
                    request.error(request.response.Error.Description, request);
                }
                else {
                    if (request.service == "PredictPlace")
                    {
                        request.success(request.response.Places, request.response, request);
                    }
                    else
                    {
                        request.success(request.response.Place, request.response, request);
                    }
            ...
            else
            {
                ...
                {
                    request.success(request.response.Addresses, request.response, request);
                }
            }
        ...
        if (request.cache)
            Vend.requestCache[request.url] = request;

        if (request.position)
            Vend.scriptRequests[request.position - 1] = null;

        if (Vend.requestQueue.length)
            processRequest(Vend.requestQueue.shift());
    }

    Vend.Request = Vend.Request || function (service, data, success, error) {
        var request = new Vend.Eventable(this);
        ...
        request.sessionId = so.sessionId || "";

        request.cache = !!request.data.$cache; 
        request.queue = !!request.data.$queue; 
        request.block = !!request.data.$block; 
        request.post = !!request.data.$post; 
        request.credentials = !!request.data.$credentials; 
        request.unwrapped = !!request.data.$unwrapped; 

        if (request.service == "PredictPlace")
        {
            request.destination = ~request.service.indexOf("//") ? request.service : Vend.protocol + "//" + options.Host + "/" + Vend.endpoint + "/" + request.service + "/json";
            request.query = "";
        ...
        else if (request.service == "SelectPlace")
        {
            request.destination = ~request.service.indexOf("//") ? request.service : Vend.protocol + "//" + options.Host + "/" + Vend.endpoint + "/" + request.service + "/json";
            request.query = "";
            for (var p in request.data)
            ...
            request.destination = ~request.service.indexOf("//") ? request.service : Vend.protocol + "//" + options.Host + "/" + Vend.endpoint + "/" + request.service;
            request.query = "";
            ...
            if (request.sessionId) {
                request.query += (request.query ? "&" : "") + "s=" + encodeURIComponent(request.sessionId);
            }
        }
    ...
    Vend.process = Vend.process || function (request) {
        processRequest(request);
    }


    Vend.fetch = Vend.fetch || function (service, data, success, error) {
        processRequest(new Vend.Request(service, data, success, error));
    }


    Vend.clearBlockingRequests = Vend.clearBlockingRequests || function () {
        Vend.waitingRequest = false;
        Vend.blockRequests = false;
    }

  
    Vend.loadScript = Vend.loadScript || function (name, callback, doc) {
        var script = Vend.create("script", { type: "text/javascript" }),
        ...
        script.src = (~name.indexOf("/") ? "" : Vend.protocol + "//" + options.Host + "/js/") + name;
        head.insertBefore(script, head.firstChild);
    }

   
    Vend.loadStyle = Vend.loadStyle || function (name, callback, doc) {
        var style = Vend.create("link", { type: "text/css", rel: "stylesheet" }),
            head = (doc || document).getElementsByTagName("head")[0];
        ...
        style.href = (~name.indexOf("/") ? "" : Vend.protocol + "//" + options.Host + "/css/") + name;
        head.insertBefore(style, head.firstChild);
    }


    Vend.Item = Vend.Item || function (data, format) {
        var item = new Vend.Eventable(this),
            highlightClass = "vendselected";

                item.data = data;

        item.format = format;
        item.html = Vend.formatLine(data, format);
        item.title = data.title || Vend.removeHtml(item.html);
        item.tag = Vend.formatTag(data.tag || item.html);
        item.element = Vend.create("div", { className: "venditem", innerHTML: item.html, title: item.title });
        item.visible = true;

        item.highlight = function () {
            Vend.addClass(item.element, highlightClass);
            item.fire("highlight");
        ...
        item.lowlight = function () {
            Vend.removeClass(item.element, highlightClass);
            item.fire("lowlight");
        ...
        item.mouseup = function () {
            ...
            if (Vend.galaxyFix) item.select();
        }

        item.click = function () {
            item.fire("click");

            if (Vend.galaxyFix) return;
        ...
        Vend.listen(item.element, "mouseover", item.mouseover);
        Vend.listen(item.element, "mouseout", item.mouseout);
        Vend.listen(item.element, "mousedown", item.mousedown);
        Vend.listen(item.element, "mouseup", item.mouseup);
        Vend.listen(item.element, "click", item.click);

        return item;
    }

    Vend.Collection = Vend.Collection || function () {

        var collection = new Vend.Eventable(this);
        ...
        collection.add = function (data, format, callback) {
            ...
            function createItem(attributes) {
                var item = new Vend.Item(attributes, format);
        ...
        * @callback Vend.Collection~itemCallback
        * @param {Object} data - The original data of the item. */

        /** Delegate function to handle an item.
        * @callback Vend.Collection~itemDelegate
        * @param {Vend.Item} item - The current item.
        ...
        * @callback Vend.Collection~itemMatcher
        * @param {Vend.Item} item - The current item.
        * @returns {boolean} Returns a response of true for a matching item. */
    }

    Vend.List = Vend.List || function (options) {
        /** @lends Vend.List.prototype */
        var list = new Vend.Eventable(this);

        list.options = options || {};
        /** The HTML parent element of the list */
        list.element = Vend.create("div", { className: "so solist" });

        list.collection = new Vend.Collection();
        list.visible = true;
        list.scroll = {
            held: false,
            moved: false,
        ...
        list.firstItem = null;
        list.lastItem = null;
        list.firstItemClass = "sofirstitem";
        list.lastItemClass = "solastitem";
        ...
        * @fires draw */
        ...
        list.markItems = function () {
            if (list.firstItem) Vend.removeClass(list.firstItem.element, list.firstItemClass);
            if (list.lastItem) Vend.removeClass(list.lastItem.element, list.lastItemClass);
            ...
            if (list.collection.count) {
                ...
                Vend.addClass(list.firstItem.element, list.firstItemClass);
                Vend.addClass(list.lastItem.element, list.lastItemClass);
            }
        }
        ...
        * Most mobile browsers will handle scrolling without this. */
        list.enableTouch = function () {
            ...
            Vend.listen(list.element, "touchstart", touchStart);
            Vend.listen(list.element, "touchmove", touchMove);
            Vend.listen(list.element, "touchend", touchEnd);
            Vend.listen(list.element, "touchcancel", touchCancel);
        ...
        * @returns {number} The height required in pixels. */
        list.getHeight = function () {
            var visibleItems = list.collection.visibleItems(),
                headerItemHeight = list.headerItem ? Vend.getSize(list.headerItem.element).height : 0,
                footerItemHeight = list.footerItem ? Vend.getSize(list.footerItem.element).height : 0,
                lastItemHeight = 0,
                itemsHeight = 0;

            //count the height of items in the list
            for (var i = 0; i < visibleItems.length && i < list.options.maxItems; i++) {
                lastItemHeight = Vend.getSize(visibleItems[i].element).height;
        ...
        function createListItem(data, format, callback) {
            var item = new Vend.Item(data, format);
        ...
        list.setHeaderItem = function (data, format, callback) {
            ...
            Vend.addClass(list.footerItem.element, "soheaderitem");
            list.markItems();
            return list;
        }
        ...
        list.setFooterItem = function (data, format, callback) {
            ...
            Vend.addClass(list.footerItem.element, "sofooteritem");
            list.markItems();
            return list;
        }
        ...
        if (list.options.name) {
            Vend.setAttributes(list.element, { id: list.options.name, role: "listbox", "aria-activedescendant": "" });

            list.collection.listen("add", function (additions) {
                function listenHighlightChange(item) {
                    item.listen("highlight", function () {
                        Vend.setAttributes(list.element, { "aria-activedescendant": item.id });
                    });
                }
                ...
                list.collection.all(function (item, index) {
                    ...
                    Vend.setAttributes(item.element, { role: "option" });
                });
            });
        }
    ...
    Vend.AutoComplete = Vend.AutoComplete || function (fields, options) {
        /** @lends Vend.AutoComplete.prototype */
        var autocomplete = new Vend.Eventable(this);
        ...
        autocomplete.element = Vend.create("div", { className: "soautocomplete sotext" });
        autocomplete.anchors = [];

        autocomplete.list = new Vend.List(autocomplete.options);
        ...
        * @type {boolean} */
        ...
        /** Header element. */
        autocomplete.header = {
            element: Vend.create("div", { className: "soheader" }),
            headerText: Vend.create("div", { className: "somessage" }),
            ...
            setText: function (text) {
                ...
                if (typeof text == 'string') {
                    Vend.clear(this.headerText);
                    this.headerText.appendChild(Vend.create("span", { className: "somessageicon" }));
                    this.headerText.appendChild(Vend.create("span", { innerHTML: text }));
                }
        ...
        /** Footer element. */
        autocomplete.footer = {
            element: Vend.create("div", { className: "sofooter" }),
        ...
        autocomplete.load = function () {
            ...
            if (fields.length && fields.constructor === Array) {
                for (var i = 0; i < fields.length; i++)
                    autocomplete.attach(Vend.getElement(fields[i]));
            }
            else
                autocomplete.attach(Vend.getElement(fields));

            Vend.listen(autocomplete.element, "mouseover", function () { autocomplete.hover = true; });
            Vend.listen(autocomplete.element, "mouseout", function () { autocomplete.hover = false; });

            //page events
            Vend.listen(document, "click", documentClicked);
            Vend.listen(window, "resize", windowResized);

            if ((document.documentMode && document.documentMode <= 7) || (/\bMSIE\s(7|6)/).test(Vend.agent))
                autocomplete.setWidth(280);

            if (document.documentMode && document.documentMode <= 5) {
                Vend.applyStyleFixes(".so .sofooter", { fontSize: "0pt" });
                Vend.applyStyleFixes(".so .soflag", { fontSize: "0pt" });
            }
        ...
        * @param {HTMLElement} field - The field to attach to. */
        autocomplete.attach = function (field) {
            ...
            function anchorToField(f) {
                ...
                //check the field
                if (!f || !f.tagName) {
                    Vend.append(autocomplete.element);
                    return;
                }
        ...
        * @param {HTMLElement} field - The field to position the list under. */
        autocomplete.position = function (field) {
            var fieldPosition = Vend.getPosition(field),
                fieldSize = Vend.getSize(field),
                topParent = Vend.getTopOffsetParent(field),
                parentScroll = Vend.getParentScroll(field),
                listSize = Vend.getSize(autocomplete.element),
                windowSize = Vend.getSize(window),
                windowScroll = Vend.getScroll(window),
                fixed = !Vend.isPage(topParent);
            ...
            var ownBorderWidth = (parseInt(Vend.getStyle(autocomplete.element, "borderLeftWidth")) + parseInt(Vend.getStyle(autocomplete.element, "borderRightWidth"))) || 0,
                preferredWidth = Math.max((Vend.getSize(field).width - ownBorderWidth), 0);
        ...
        * @param {number} [position] - The index at which to set the carat. */
        autocomplete.prompt = function (text, position) {
            if (typeof position == "number") {
                ...
                Vend.setValue(autocomplete.field, text);
        ...
        autocomplete.handleKey = function (key, event) {
            if (key === 27) { //escape
                ...
                if (autocomplete.visible && autocomplete.list.navigate(key)) {
                    if (event) Vend.smash(event); //keys handled by the list, stop other events
                }
        ...
        * @fires filter */
        autocomplete.filter = function (term) {
            term = term || Vend.getValue(autocomplete.field);
        ...
        autocomplete.destroy = function () {
            Vend.remove(autocomplete.element);

            //stop listening to page events
            Vend.ignore(document, "click", documentClicked);
            Vend.ignore(window, "resize", windowResized);

            for (var i = 0; i < autocomplete.fieldListeners.length; i++)
                Vend.ignore(autocomplete.fieldListeners[i].field, autocomplete.fieldListeners[i].event, autocomplete.fieldListeners[i].action);
        }
        ...
        if (autocomplete.options.className) Vend.addClass(autocomplete.element, autocomplete.options.className);
    ...
    Vend.Modal = Vend.Modal || function (options) {
        /** @lends Vend.Modal.prototype */
        var modal = new Vend.Eventable(this);

        modal.options = options || {};

        /** The parent HTML element of the modal window */
        modal.element = Vend.create("div", { className: "somodal" });
        modal.border = Vend.create("div", { className: "soborder" });
        modal.frame = Vend.create("div", { className: "soframe" });
        modal.content = Vend.create("div", { className: "socontent sotext" });
        modal.mask = Vend.create("div", { className: "sofullscreen somask" });
        modal.form = [];

        /** Header element. */
        modal.header = {
            element: Vend.create("div", { className: "soheader" }),
            headerText: Vend.create("div", { className: "sotitle" }, modal.options.titleStyle || ""),
        ...
        * @param {HTMLElement} The HTML field created. */
        modal.addField = function (labelText, properties) {
            ...
            var row = Vend.create("div", { className: "soinputrow" }),
                input = Vend.create(properties.tag || "input", properties),
                label = Vend.create("label", { htmlFor: input.id || "", innerHTML: labelText || "" });
        ...
        * @return {Array.<HTMLElement>} The two HTML fields created. */
        modal.addHalfFields = function (labelText, firstProperties, secondProperties) {
            ...
            var row = Vend.create("div", { className: "soinputrow" }),
                firstInput = Vend.create(firstProperties.tag || "input", firstProperties),
                secondInput = Vend.create(secondProperties.tag || "input", secondProperties),
                label = Vend.create("label", { htmlFor: firstInput.id || "", innerHTML: labelText || "" });

            Vend.addClass(firstInput, "sohalf");
            Vend.addClass(secondInput, "sohalf");
        ...
        * @returns {HTMLElement} The HTML input element created. */
        modal.addButton = function (labelText, callback, floatRight) {
            var button = Vend.create("input", { type: "button", value: labelText, className: "sobutton" });
            ...
            function click() {
                ...
                for (var i = 0; i < modal.form.length; i++)
                    details[modal.form[i].label] = Vend.getValue(modal.form[i].element);
            ...
            Vend.listen(button, "click", click);
        ...
        modal.centre = function () {
            var modalSize = Vend.getSize(modal.element);
        ...
        * @fires show */
        modal.show = function () {
            ...
            if (!(document.documentMode && document.documentMode <= 5) && !(/\bMSIE\s6/).test(Vend.agent)) {
        ...
        * @fires clear */
        ...
        Vend.listen(modal.mask, "click", modal.hide);
        ...
        Vend.append(modal.mask);
        Vend.append(modal.element);
    ...
    * @param {string} message - The text to show. */
    Vend.Tooltip = Vend.Tooltip || function (element, message) {
        /** @lends Vend.Tooltip.prototype */
        var tooltip = new Vend.Eventable(this);

        /** The parent HTML element for the tooltip. */
        tooltip.element = Vend.create("div", { className: "sotooltip" });
        tooltip.background = Vend.create("div", { className: "sobackground" });
        tooltip.message = Vend.create("div", { className: "somessage", innerText: message });
        ...
        * @param {string} text - The text to set. */
        tooltip.setMessage = function (text) {
            Vend.setValue(tooltip.message, text);
        }

        /** Positions the tooltip centrally above the element. */
        tooltip.position = function () {
            var parentPosition = Vend.getPosition(element),
                parentSize = Vend.getSize(element),
                topParent = Vend.getTopOffsetParent(element),
                messageSize = Vend.getSize(tooltip.message),
                windowSize = Vend.getSize(window),
                windowScroll = Vend.getScroll(window),
                fixed = !Vend.isPage(topParent);
        ...
        if (element = Vend.getElement(element)) {
            Vend.listen(element, "mouseover", tooltip.show);
            Vend.listen(element, "mouseout", tooltip.hide);
        }
    ...
    * @example Vend.formatLine({"line1": "Line One", "line2": "Line Two"}, "{line1}{, {line2}}");
    * @returns "Line One, Line Two" */
    Vend.formatLine = Vend.formatLine || function (item, format) {
    ...
    * @returns {string} The formatted tag. */
    Vend.formatTag = Vend.formatTag || function (line) {
        return line ? Vend.replaceList(Vend.replaceList(Vend.removeHtml(line.toUpperCase()), Vend.diacritics), Vend.synonyms) : "";
    }
    ...
    * @returns {Array.<string>} The formatted tag words. */
    Vend.formatTagWords = Vend.formatTagWords || function (line) {
        return Vend.formatTag(line).split(" ");
    }
    ...
    * @returns {string} The formatted text. */
    Vend.formatCamel = Vend.formatCamel || function (line, separator) {
        separator = separator || " ";
    ...
    * @returns {string} The formatted text. */
    Vend.replaceList = Vend.replaceList || function (line, list) {
    ...
    * @param {string} line - The text to format.
    * @returns {string} The formatted text. */
    Vend.removeHtml = Vend.removeHtml || function (line) {
    ...
    * @param {string} line - The text to format.
    * @returns {string} The formatted text. */
    Vend.escapeHtml = Vend.escapeHtml || function (line) {
        return Vend.replaceList(line, Vend.hypertext);
    }
    ...
    * @param {string} line - The text to format.
    * @returns {string} The formatted text. */
    Vend.validId = Vend.validId || function (line) {
    ...
    * @param {string} line - The text to format.
    * @returns {string} The formatted text. */
    Vend.trimSpaces = Vend.trimSpaces || function (line) {
    ...
    * @returns {string} The formatted text. */
    Vend.tidy = Vend.tidy || function (line, symbol) {
    ...
    * @returns {string} The text. */
    Vend.getText = Vend.getText || function (line) {
    ...
    * @returns {string} The number. */
    Vend.getNumber = Vend.getNumber || function (line) {
        return /\d+/.exec(line);
    }
    ...
    * @returns {Object} The object based on the JSON. */
    Vend.parseJSON = Vend.parseJSON || function (text) {
    ...
    * @returns {Date} The date object. */
    Vend.parseJSONDate = Vend.parseJSONDate || function (text) {
        return new Date(parseInt(Vend.getNumber(text)));
    }
    ...
    * @returns {boolean} True if the text contains the word. */
    Vend.containsWord = Vend.containsWord || function (text, word) {
    ...
    * @returns {string} The text with the word replaced. */
    Vend.removeWord = Vend.removeWord || function (text, word) {
    ...
    * @returns {Object} The destination object. */
    Vend.merge = Vend.merge || function (source, destination) {
        for (var i in source)
    ...
    * @returns {?HTMLElement} The first element found or null. */
    Vend.getElement = Vend.getElement || function (reference, base) {
        if (!reference)
            return null;
        ...
        if (typeof reference == "string") {
            base = Vend.getElement(base) || document;
        ...
        return Vend.fuzzyMatch ? Vend.getElementByRegex(reference, base) : null;
    }
    ...
    * @returns {HTMLElement} The first element found or null. */
    Vend.getElementByRegex = Vend.getElementByRegex || function (regex, base) {
        ...
        if (regex && typeof regex == "object" && regex.constructor === RegExp) {
            ...
            for (var t = 0; t < Vend.fuzzyTags.length; t++) {
                var elements = base.getElementsByTagName(Vend.fuzzyTags[t]);
    ...
    * @returns {string} The value of the element. */
    Vend.getValue = Vend.getValue || function (element) {
        if (element = Vend.getElement(element)) {
    ...
    * @param {*} value - The value to set. */
    Vend.setValue = Vend.setValue || function (element, value) {
        if ((value || value === "") && (element = Vend.getElement(element))) {
            var valueText = value.toString(),
                valueTextMatch = Vend.formatTag(valueText);
            ...
            if (element.tagName === "INPUT") {
                if (element.type === "checkbox")
                    ...
                    var group = document.getElementsByName(element.name);
                    for (var r = 0; r < group.length; r++) {
                        if (Vend.formatTag(group[r].value) === valueTextMatch) {
                    ...
                    element.value = Vend.tidy(valueText.replace(/\\n|\n/gi, ", "), ", ");
            }
            else if (element.tagName === "TEXTAREA")
                element.value = valueText.replace(/\\n|\n/gi, "\n");
            else if (element.tagName === "SELECT") {
                for (var s = 0; s < element.options.length; s++) {
                    if (Vend.formatTag(element.options[s].value) === valueTextMatch || Vend.formatTag(element.options[s].text) === valueTextMatch) {
    ...
    * @returns {boolean} True if the element supports text input. */
    Vend.inputField = Vend.inputField || function (element) {
        if (element = Vend.getElement(element))
    ...
    * @returns {boolean} True if the element in a select list field. */
    Vend.selectList = Vend.selectList || function (element) {
        if (element = Vend.getElement(element))
    ...
    * @returns {HTMLOptionElement} The current selected item. */
    Vend.getSelectedItem = Vend.getSelectedItem || function (element) {
        if ((element = Vend.getElement(element)) && element.tagName === "SELECT" && element.selectedIndex >= 0)
    ...
    * @returns {boolean} True if the element in a checkbox. */
    Vend.checkBox = Vend.checkBox || function (element) {
        if (element = Vend.getElement(element))
    ...
    * @param {string|HTMLElement} element - The element to clear. */
    Vend.clear = Vend.clear || function (element) {
        Vend.setValue(element, "");
        return Vend;
    }
    ...
    * @returns {Object} The top and left of the position. */
    Vend.getPosition = Vend.getPosition || function (element) {
        ...
        if (element = Vend.getElement(element)) {
            ...
            if (typeof element.getBoundingClientRect != 'undefined') {
                var bb = element.getBoundingClientRect(),
                    fixed = !Vend.isPage(Vend.getTopOffsetParent(element)),
                    pageScroll = Vend.getScroll(window),
                    parentScroll = Vend.getParentScroll(element);
    ...
    Vend.isPage = Vend.isPage || function (element) {
    ...
    * @returns {Object} The top and left of the scroll. */
    Vend.getScroll = Vend.getScroll || function (element) {
        return {
            left: parseInt(element.scrollX || element.scrollLeft, 10) || (Vend.isPage(element) ? parseInt(document.documentElement.scrollLeft) || 0 : 0),
            top: parseInt(element.scrollY || element.scrollTop, 10) || (Vend.isPage(element) ? parseInt(document.documentElement.scrollTop) || 0 : 0)
        };
    }
    ...
    * @returns {Object} The height and width of the element. */
    Vend.getSize = Vend.getSize || function (element) {
        return {
            height: (element.offsetHeight || element.innerHeight || (Vend.isPage(element) ? (document.documentElement.clientHeight || document.body.clientHeight) : 0)),
            width: (element.offsetWidth || element.innerWidth || (Vend.isPage(element) ? (document.documentElement.clientWidth || document.body.clientWidth) : 0))
        };
    }
    ...
    * @returns {Object} The top and left of the scroll. */
    Vend.getParentScroll = Vend.getParentScroll || function (element) {
        var empty = { left: 0, top: 0 };
    ...
    * @returns {HTMLElement} The element controlling the relative position. */
    Vend.getTopOffsetParent = Vend.getTopOffsetParent || function (element) {
        while (element.offsetParent) {
            element = element.offsetParent;

            //fix for Firefox
            if (Vend.getStyle(element, "position") === "fixed")
                break;
        }
    ...
    * @returns {string} The value of the style property. */
    Vend.getStyle = Vend.getStyle || function (element, property) {
    ...
    * @param {string} className - The name of the style class to add. */
    Vend.addClass = Vend.addClass || function (element, className) {
        if (element = Vend.getElement(element)) {
            if (!Vend.containsWord(element.className || "", className))
    ...
    * @param {string} className - The name of the style class to remove. */
    Vend.removeClass = Vend.removeClass || function (element, className) {
        if (element = Vend.getElement(element))
            element.className = Vend.removeWord(element.className, className);
    }
    ...
    * @param {Object} attribute - The value to set. */
    Vend.setAttribute = Vend.setAttribute || function (element, attribute, value) {
        if (element = Vend.getElement(element))
    ...
    * @param {Object} attributes - The element attributes and values to set. */
    Vend.setAttributes = Vend.setAttributes || function (element, attributes) {
        if (element = Vend.getElement(element)) {
    ...
    * This will add them to the fixes list for Vend.reapplyStyleFixes.
    * @memberof Vend
    ...
    * @param {Object} fixes - An object with JavaScript style property name and value. */
    Vend.applyStyleFixes = Vend.applyStyleFixes || function (selectorText, fixes) {
        ...
        Vend.styleFixes.push({ selectorText: selectorText, fixes: fixes });
    }

    /** Reapplies all fixes to style sheets added by Vend.applyStyleFixes.
    * @memberof Vend */
    Vend.reapplyStyleFixes = Vend.reapplyStyleFixes || function () {
        var fixesList = Vend.styleFixes;

        Vend.styleFixes = [];

        for (var i = 0; i < fixesList.length; i++)
            Vend.applyStyleFixes(fixesList[i].selectorText, fixesList[i].fixes);
    }

    /** Creates a style sheet from cssText.
    * @memberof Vend
    * @param {string} cssText - The CSS text for the body of the style sheet. */
    Vend.createStyleSheet = Vend.createStyleSheet || function (cssText) {
        if (document.createStyleSheet)
            document.createStyleSheet().cssText = cssText;
        else
            document.head.appendChild(Vend.create("style", { type: "text/css", innerHTML: cssText }));
    }
    ...
    * @returns {HTMLElement} The created element. */
    Vend.create = Vend.create || function (tag, properties, cssText) {
    ...
    /** Adds an element to the Vend container on the page.
    * If the container does not exist it is created.
    * @memberof Vend
    * @param {HTMLElement} element - The element to add to the container. */
    Vend.append = Vend.append || function (element) {
        if (!Vend.container) {
            Vend.container = Vend.create("div", { className: "so" });
            document.body.appendChild(Vend.container);
        }

        Vend.container.appendChild(element);
    }

    /** Removes an element from the container on the page.
    * @memberof Vend
    * @param {HTMLElement} element - The element to remove from the container. */
    Vend.remove = Vend.remove || function (element) {
        if (element && element.parentNode && element.parentNode === Vend.container)
            Vend.container.removeChild(element);
    }


    Vend.listen = Vend.listen || function (target, event, action, capture) {
    ...
    * @returns {boolean} False is the event was stopped by any of its handlers. */
    Vend.fire = Vend.fire || function (target, event) {
    ...
    Vend.ignore = Vend.ignore || function (target, event, action) {
    ...
    * @param {Event} event - The event to stop. */
    Vend.smash = Vend.smash || function (event) {
    ...
    * @param {string} message - The debug message text. */
    Vend.debug = Vend.debug || function (message) {
    ...
    * @param {integer} immediate - An ovveride to call the function immediately. */
    ...
    Vend.debounce = Vend.debounce || function (func, wait, immediate) {
        var timeout;
        return function () {
            var context = this;
    ...
    * @param {function} func - The function to check */
    Vend.defined = Vend.defined || function (func) {
    ...
    * @param {string} elementNameOrId - The name or ID of the DOM element. */
    ...
    Vend.getReferenceToElement = Vend.getReferenceToElement || function (element) {
     ...
     * @returns {Object} The destination object.
     */
    Vend.extend = Vend.extend || function (obj /*...*/) {
     ...
     * @returns {}
     */
    Vend.getStyle = Vend.getStyle || function (element, styleProperty) {
     ...
     */
    Vend.supports = Vend.supports || function (checkType) {
    ...
    Vend.guid = Vend.guid || (function () {
        ...
        return function () {
            return s4() + s4() + 'S' + s4() + 'O' + s4() + 'A' +
                s4() + 'C' + s4() + s4() + s4();
        };
    })();

    Vend.sessionId = Vend.sessionId || Vend.guid();
...
(function () {
    ...
    * @enum {number} */
    ...
    Vend.CountryList = function (fields, options) {
        /** @lends Vend.CountryList.prototype */
        var countrylist = new Vend.Eventable(this);
        ...
        * @type {Array.<Object>} */
        ...
        countrylist.options.nameType = countrylist.options.nameType || Vend.countryNameType.NAME;
        countrylist.options.valueType = countrylist.options.valueType || Vend.countryNameType.NAME;
        countrylist.options.fallbackCode = countrylist.options.fallbackCode || "USA";

        countrylist.autocomplete = new Vend.AutoComplete(countrylist.fields, countrylist.options.list);
        ...
        countrylist.template = "<div class='soflag'></div><div class='soflaglabel'>{" + countrylist.nameProperty + "}</div>";

        countrylist.load = function () {
            Vend.addClass(countrylist.autocomplete.element, "socountrylist");
            ...
            if (countrylist.options.codesList) {
                ...
                for (var i = 0; i < codesSplit.length; i++) {
                    ...
                    for (var c = 0; c < Vend.countries.length; c++) {
                        if (Vend.countries[c].iso2 === code || Vend.countries[c].iso3 === code) {
                            filteredList.push(Vend.countries[c]);
                            break;
                        }
                    }
                }
                ...
                if (countrylist.options.fillOthers) {
                    for (var o = 0; o < Vend.countries.length; o++) {
                        var contains = false;

                        for (var f = 0; f < filteredList.length; f++) {
                            if (Vend.countries[o].iso3 === filteredList[f].iso3)
                                contains = true;
                        }

                        if (!contains) filteredList.push(Vend.countries[o]);
                    }
                }

                countrylist.autocomplete.clear().add(filteredList, countrylist.template, selectCountry);
            }
            else countrylist.autocomplete.clear().add(Vend.countries, countrylist.template, selectCountry);
            ...
            countrylist.autocomplete.list.collection.all(function (item) {
                ...
                item.tag += " " + Vend.formatTag(item.data.iso3 + (item.data.alternates ? " " + item.data.alternates.join(" ") : ""));
            });
            ...
            function textChanged(field) {
                ...
                if (Vend.selectList(field)) {
                    var selected = Vend.getSelectedItem(field);
                    countrylist.change(countrylist.find(selected.value) || countrylist.find(selected.text));
                }
                else
                    countrylist.setCountry(Vend.getValue(field));
        ...
        * @fires populate */
        countrylist.populate = function () {
            ...
            for (var i = 0; i < countrylist.fields.length; i++) {
                var countryField = Vend.getElement(countrylist.fields[i]),
                    currentValue = Vend.getValue(countryField);

                Vend.setValue(countryField, (Vend.selectList(countryField) ? value : name));

                if (countrylist.options.prepopulate && currentValue !== Vend.getValue(countryField))
                    Vend.fire(countryField, "change");
            }
        ...
        * @returns {HTMLDivElement} A dynamic HTML DIV showing the flag as an icon. */
        countrylist.flag = function () {
            var flag = Vend.create("div", { className: "soflag" });
        ...
        * @param {string} key - A license key for the request. */
        countrylist.setCountryByIP = function (key) {
            function success(response) {
                if (response.Iso3)
                    countrylist.setCountry(response.Iso3, true);
            }

            if (key) Vend.fetch("Country", { id: key }, success);
        }
...
(function () {
    ...
    * @memberof Vend */
    ...
    //    "SubPremise": "73",
    //    "PremiseNumber": "",
    //    "PremiseName": "Thirlestane",
    //    "SecondaryStreetName": "",
    //    "StreetLine": "Lemsford Road",
    //    "StreetName": null,
    //    "Locality": "St. Albans",
    //    "Address1": "73 Thirlestane",
    //    "Address2": "Lemsford Road",
    //    "Address3": "",
    //    "Address4": "",
    //    "Address5": "",
    //    "AdminArea": "Hertfordshire",
    //    "AdminAreaCode": "",
    //    "PostalCode": "AL1 3PD",
    ...
    //    "CountryIso3": "GBR",
    //    "POBox": "",
    //    "FormattedAddress": "73 Thirlestane\nLemsford Road\nST. ALBANS\nAL1 3PD\nUNITED KINGDOM",
    //    "AddressType": "Residential",
    //    "ResolutionLevel": "Premise",
    //    "BarcodeDigits": null,
    //    "CarrierRoute": null,
    //    "CountyName": null,
    //    "CountyCode": null,
    ...
    //    "DPVDesc": null,
    //    "AddressNotes": null,
    //    "AddressNotesDesc": null,
    //    "Corrections": null,
    //    "CorrectionsDesc": null,
    ...
    * @memberof Vend */
    Vend.templates = {
        AUTOCOMPLETE: "{HighlightedText}{<span class='sodescription'>{HighlightedDescription}</span>}",
        AUTOCOMPLETE_UTILITY: "{<span class='soutilitytype'>({UtilityType})</span>}{HighlightedText}{<span class='sodescription'>{HighlightedDescription}</span>}"
    };

    
    Vend.Address = function (fields, options) {
        /** @lends Vend.Address.prototype */

        function parseOptions(options) {
            options = options || {};
            options.name = options.name || "";
            //options.source = options.source || "";
            options.isTrial = typeof (options.isTrial) == "boolean" ? options.isTrial : true;
            options.DoPlace = typeof (options.DoPlace) == "boolean" ? options.DoPlace : false;
            options.SearchType = options.SearchType || "Locality";
            ...
            options.brand = options.brand || "Service Objects" || "Service Objects";
            options.product = options.product || "DOTS Global Address Complete";
            options.culture = options.culture || "en-US";
            ...
            options.filteringMode = options.filteringMode || Vend.filteringMode.EVERYTHING;
            options.orderingMode = options.orderingMode || Vend.orderingMode.DEFAULT;
            ...
            options.countries.list = options.countries.list || Vend.extend({}, options.list);
            ...
            options.bar.logoClass = options.bar.logoClass || "sologo" || "sologo";
            ...
            options.Host = options.isTrial ? Vend.host_trial : Vend.host_live;
            
            options.search.setBiasToIP = typeof (options.setBiasToIP) == "boolean" ? options.setBiasToIP : false;
            options.search.Validate = typeof (options.Validate) == "boolean" ? options.Validate : true;
            
        };

        parseOptions(options);

        var address = new Vend.Eventable(this);
        ...
        * @fires load */
        address.load = function () {
            ...
            for (var f = 0; f < address.fields.length; f++) {
                ...
                field.mode = typeof (field.mode) == "number" ? field.mode : Vend.fieldMode.DEFAULT;

                if (field.mode & Vend.fieldMode.COUNTRY) {
                    countryFields.push(field.element);

                    //tell the countrylist to use the same format
                    if (/CountryIso2/.test(field.field)) {
                        address.options.countries.nameType = address.options.countries.nameType || Vend.countryNameType.ISO2;
                        address.options.countries.valueType = address.options.countries.valueType || Vend.countryNameType.ISO2;
                    }
                    if (/CountryIso3/.test(field.field)) {
                        address.options.countries.nameType = address.options.countries.nameType || Vend.countryNameType.ISO3;
                        address.options.countries.valueType = address.options.countries.valueType || Vend.countryNameType.ISO3;
                    }
                }
                else if (field.mode & Vend.fieldMode.SEARCH) {
                    searchFields.push(field.element);

                    if (address.options.suppressAutocomplete) {
                        var elem = Vend.getElement(field.element);
            ...
            address.autocomplete = new Vend.AutoComplete(searchFields, address.options.list);
            ...
            address.autocomplete.checkIfProbablyUtilitySearch = function () {
                ...
                if (address.options.utilitiesenabled) {
                    if (Vend.getValue(address.autocomplete.field).replace(/[^0-9]/g, "").length >= 5) {
            ...
            address.autocomplete.list.listen("filter", function () {
                if (address.frugalSearch)
                    address.search(Vend.getValue(address.autocomplete.field));
            });
            ...
            if (!address.options.countries.value && countryFields.length)
                address.options.countries.value = Vend.getValue(countryFields[0]);
            ...
            address.countrylist = new Vend.CountryList(countryFields, address.options.countries);
            address.countrylist.autocomplete.options.emptyMessage = Vend.messages[address.language].NOCOUNTRY;
            ...
            address.countrylist.autocomplete.listen("hide", function () {
                ...
                if (address.storedSearch != null)
                    Vend.setValue(address.autocomplete.field, address.storedSearch);
            ...
            if (address.options.GeoLocationEnabled) {
                var reverseButton = Vend.create("div", { className: "geoLocationIcon", title: Vend.messages[address.language].GEOLOCATION });
                var reverseText = Vend.create("div", { className: "geoLocationMessage", innerHTML: Vend.messages[address.language].GEOLOCATION });
                Vend.listen(reverseButton, "click", address.startGeoLocation);
                Vend.listen(reverseText, "click", address.startGeoLocation);
            ...
            var flagbutton = Vend.create("div", { className: "soflagbutton" }),
            ...
            Vend.listen(flagbutton, "click", address.switchToCountrySelect);

            //add the country select message to the footer - shown by default
            var message = Vend.create("div", { className: "somessage sodisableselect", innerHTML: Vend.messages[address.language].COUNTRYSELECT });
            if (address.options.bar.showCountry) {
                address.autocomplete.footer.setContent(message);
            }

            //add the logo to the footer - shown with results
            var link = Vend.create("a", { href: address.options.bar.logoUrl, target: "_blank", rel: "nofollow" }),
                logo = Vend.create("div", { className: (address.options.bar.logoClass + " sologo" + address.language), title: address.options.bar.logoTitle });
            ...
            var countryMessage = Vend.create("div", { className: "somessage sodisableselect", innerHTML: Vend.messages[address.language].COUNTRYSELECT });
            if (address.options.bar.showCountry) {
            ...
            if (options.name) {
                ...
                Vend.setAttributes(message, { id: listname + "_label" });
                Vend.setAttributes(flagbutton, { id: listname + "_button", role: "button", "aria-labelledby": listname + "_label" });
                Vend.setAttributes(countryMessage, { id: countrylistname + "_label" });
            }

            //create the hovering message box
            address.messageBox = Vend.create("div", { className: "sotext sonotification" });
            Vend.append(address.messageBox, Vend.container);
        ...
        address.searchFromField = function () {
            var term = Vend.getValue(address.autocomplete.field);
        ...
        address.updateGeoLocationActive = function () {
            if (address.country == "USA" && Vend.supports("reverseGeo") && address.options.GeoLocationEnabled) {
                if (address.geocodeButton) {
                    Vend.addClass(address.geocodeButton, "active");
                    Vend.addClass(address.geocodeText, "active");
                }
            } else {
                if (address.geocodeButton) {
                    Vend.removeClass(address.geocodeButton, "active");
                    Vend.removeClass(address.geocodeText, "active");
                }
            }
        };
        ...
        address.geolocationLookup = function (location) {
            if (location.coords) {
                ...
                Vend.fetch("Capture/Interactive/GeoLocation/v1.00", params, function (items, response) {
                    //success
                    Vend.removeClass(address.geocodeButton, "working");
                    if (items.length)
                        address.display(items, Vend.templates.AUTOCOMPLETE, response);
                    else
        ...
        address.utilitiesLookup = function (utilityCode) {
            ...
            Vend.fetch("Capture/Interactive/Utilities/v1.00", params, function (items, response) {
                // Success
                if (items.length)
                    address.display(items, Vend.templates.AUTOCOMPLETE_UTILITY, response);
                else
                    address.noResultsMessage();
            }, function (err) {
                address.error(err);
        ...
        * @fires search */
        address.search = function (term) {
            ...
            if (address.autocomplete.addUtilityLookupToTop) {
                ...
                ], Vend.templates.AUTOCOMPLETE_UTILITY, null);
            }
            else {
                ...
                var search = {
                    ...
                    IPOrigin: address.options.search.setBiasToIP,
                    ST: address.options.SearchType
                };
                ...
                if (search.text) {
                    ...
                    if (search.container == "") {
                        if (address.options.DoPlace)
                        {
                            searchType = "PredictPlace";
                            searchParameters = { id: address.key, q: search.text, c: search.origin, SearchType: search.ST };
                        }else
                        {
                            searchType = "Predict";
                            searchParameters = { id: address.key, q: search.text, bl: search.origin, al: search.countries, l: search.limit, pl: search.language, iip: iip};
                        }
                    }
                    else {
                        searchType = "Units";
                        searchParameters = { id: address.key, q: search.container, bl: search.origin, al: search.countries, l: search.limit, pl: search.language};
                    }
                    ...
                    Vend.fetch(searchType, searchParameters, success, address.error);
                }
            }
        ...
        * @param {string} id - The address id to retrieve. */
        address.retrieve = function (id) {
            if (address.options.DoPlace) {
                ...
                var y = id.substring(id.indexOf("||||") + 4);



                var params = { id: address.key, pid: x, pt: y };

                function fail(message) {
                    address.message(Vend.messages[address.language].RETRIEVEERROR, { clickToDisable: address.options.manualEntry, error: true, clearList: true });
                ...
                function success(response, responseObject, request) {
                    ...
                    if (request.service == "SelectPlace")
                    {
                        address.populate(response);
                    }
                    else
                    {
                        address.populate(request.data);
                    }
                }
                ...
                Vend.fetch("SelectPlace", params, success, fail);
            }
            else {
                var params = { id: address.key, q: id };
                ...
                if (options.search.Validate) {
                    Vend.fetch("SelectValidate", params, success, fail);
                }
                else {
                    Vend.fetch("Select", params, success, fail);
                }
            }
        };
        ...
        * @throws The error. */
        address.error = function (message) {
            ...
            Vend.clearBlockingRequests();

            //if the error message is not handled throw it
            if (!address.listeners["error"]) {
                if (typeof (console) != "undefined" && typeof (console.error) != "undefined")
                    console.error(Vend.messages[address.language].SERVICEERROR + " " + message);
                else
                    throw Vend.messages[address.language].SERVICEERROR + " " + message;
            }
       ...
       * @fires display */
        address.display = function (results, template, attributes) {
            ...
            address.autocomplete.list.collection.all(function (item) {
                if (item.data && item.data.Type && item.data.Type !== "Address") Vend.addClass(item.element, "soexpandable");
            });
            ...
            if (address.options.prompt) {
                function showPromptMessage() {
                    address.message(Vend.messages[address.language].KEEPTYPING);
                }
        ...
        address.message = function (text, messageOptions) {
            ...
            if (messageOptions.inline) {
                ...
                if (messageOptions.clickToDisable)
                    address.autocomplete.header.clear().setContent(Vend.create("div", { className: "somessage", innerHTML: text, onclick: address.manualEntry }, "cursor:pointer;")).show();
                else
                ...
                Vend.addClass(address.messageBox, "sovisible");

                Vend.removeClass(address.messageBox, "soerror");
                if (messageOptions.error) Vend.addClass(address.messageBox, "soerror");

                if (address.notifcationTimer) window.clearTimeout(address.notifcationTimer);
                Vend.removeClass(address.messageBox, "sofade");
                address.notifcationTimer = window.setTimeout(function () {
                    Vend.addClass(address.messageBox, "sofade");
                    window.setTimeout(function () {
                        Vend.removeClass(address.messageBox, "sovisible");
                    }, 500);
                }, messageOptions.notificationTimeout);

                var fieldPosition = Vend.getPosition(address.autocomplete.field),
                    fieldSize = Vend.getSize(address.autocomplete.field),
                    messageSize = Vend.getSize(address.messageBox);
        ...
        address.noResultsMessage = function () {
            address.reset();
            address.message(Vend.messages[address.language].NORESULTS, { clickToDisable: address.options.manualEntry, error: true, clearList: true });
            address.fire("noresults");
        };

        // Show the number of results possible
        address.resultCountMessage = function (count) {
            address.message(Vend.formatLine({ count: count }, Vend.messages[address.language].RESULTCOUNT));
        };
        ...
        * @param {Object} suggestion - The selected item from a find service response. */
        address.select = function (suggestion) {
            ...
            function filterSearch() {
                var searchText = Vend.getValue(address.autocomplete.field);

                if (address.options.setCursor) {
                    searchText = Vend.removeHtml(suggestion.Prediction).replace("...", "");
                    address.setCursorText(searchText, suggestion.Cursor >= 0 ? suggestion.Cursor : null);
                }
                else {
                    Vend.setValue(address.autocomplete.field, searchText + " ");
                    address.autocomplete.field.focus();
                }

                address.searchContext = { id: suggestion.Id, text: suggestion.Prediction, search: searchText };
            ...
            if (address.options.DoPlace) {
                address.retrieve(suggestion.PID + "||||" + suggestion.Prediction);
            }
            else {
                if (suggestion.AddressType === "Address") {
                    address.retrieve(suggestion.Id);
                }
                else if (suggestion.AddressType === "Utility") {
                    var searchText = Vend.getValue(address.autocomplete.field);
        ...
        * @param {string} [suffix=</b>] - The string to insert at the end of a highlight. */
        address.highlight = function (suggestions, prefix, suffix) {
            ...
            for (var s = 0; s < suggestions.length; s++) {
                ...
                suggestion.HighlightedText = suggestion.title = suggestion.tag = suggestion.Prediction;
                suggestion.HighlightedDescription = suggestion.Extra;
        ...
        * @fires populate */
        address.populate = function (items) {
            ...
            for (var a = 0; a < address.fields.length && address.options.populate; a++) {
                ...
                if (!(field.mode & Vend.fieldMode.POPULATE)) continue;

                //skip the field if it's not an input field and the onlyInputs option is set
                if (address.options.onlyInputs && !(Vend.inputField(field.element) || Vend.selectList(field.element) || Vend.checkBox(field.element))) continue;

                //skip this field if it's in preserve mode, already had a value and is not the search field
                if ((field.mode & Vend.fieldMode.PRESERVE) && Vend.getValue(field.element) && address.autocomplete.field !== Vend.getElement(field.element)) continue;

                //process format strings and/or field names
                if (!address.options.DoPlace) {
                    var format = address.fields[a].field.replace(/(Formatted)?Line/g, "FormattedLine"),
                        value = (/[\{\}]/).test(format) || format === "" ? Vend.formatLine(detail, format) : detail[format];

                    Vend.setValue(field.element, value);
                }
                else
                {
                    Vend.setValue(field.element, items.Label);
                }
            }
        ...
        * @returns {string} The formatted address line. */
        ...
        address.switchToCountrySelect = function () {
            ...
            address.storedSearch = Vend.getValue(address.autocomplete.field);
            Vend.clear(address.autocomplete.field);
            address.autocomplete.field.focus();
        };
        ...
        address.startGeoLocation = function () {
            if (Vend.supports("reverseGeo")) {
                var nav = window.navigator;
                Vend.addClass(address.geocodeButton, "working");
        ...
        * @param {string} country - The country name or code to change to. */
        ...
        address.preventAutocomplete = function (element) {
            if (element) {
                var isSet = false;
                if (Vend.browser && Vend.browser.name) {
                    switch (Vend.browser.name) {
                        case "Chrome":
                            if (Vend.browser.version && !isNaN(Number(Vend.browser.version))) {
                                var version = Number(Vend.browser.version);
                                if (version === 63) {
                                    element.autocomplete = "so-override";
                                    isSet = true;
                                }
                            }
                            break;
        ...
        address.detectLanguage = function () {
            ...
            if (!Vend.messages[address.language])
                address.language = "en";
        };
        ...
        * @fires clear */
        address.clear = function () {
            for (var a = 0; a < address.fields.length; a++)
                Vend.setValue(address.fields[a].element, "");
        ...
        * @param {string} [message] - The text to display. */
        address.addManualEntryItem = function (message) {
            message = message || Vend.messages[address.language].MANUALENTRY;
        ...
        * @returns {boolean} True if the control is bound to that element. */
        address.bound = function (element) {
            if (element = Vend.getElement(element)) {
                for (var f = 0; f < address.fields.length; f++) {
                    if (element == Vend.getElement(fields[f].element))
                        return true;
                }
            }
        ...
        * @returns {string} The standardised format string. */
        address.checkFormat = function (format) {
            function standardField(field) {
                for (var i in Vend.exampleAddress) {
