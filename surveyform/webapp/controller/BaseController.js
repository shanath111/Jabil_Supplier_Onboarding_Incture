            ...
            getTaskDetails: function (taskId, oDeferred, oi18n) {
                ...
                oModel.attachRequestCompleted(function (oEvent) {
                    if (oEvent.getParameter("success")) {
                        ...
                        if (oEvent.getSource().getData().supplierDetails) {
                            that.getView().getModel("oUserModel").setProperty("/supplierEmail", oEvent.getSource().getData().supplierDetails.email);
                        } else {
                            that.getView().getModel("oUserModel").setProperty("/supplierEmail", "");
                        }
                        ...
                        if (oEvent.getSource().getData().isBuyerReviewRejected) {
                            ...
                            that.getView().getModel("oUserModel").setProperty("/isBuyerRejectTaskTxtVis", oEvent.getSource().getData().isBuyerReviewRejected);
                        } else {
                            that.getView().getModel("oUserModel").setProperty("/isBuyerRejectTask", null);
                            that.getView().getModel("oUserModel").setProperty("/isBuyerRejectTaskTxtVis", false);
                        }
                        that.getView().getModel("oUserModel").refresh();
                        fDeferred.resolve();
