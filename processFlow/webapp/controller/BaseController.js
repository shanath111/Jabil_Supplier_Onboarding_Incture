sap.ui.define([
	...
	// "oneapp/incture/report/reports/util/taskManagement",
	// "oneapp/incture/report/reports/util/workbox",
	// "oneapp/incture/report/reports/util/utility",
	...
	return Controller.extend("oneapp.incture.processFlow.processFlow.controller.BaseController", {
		formatter: formatter,
		utility: utility,
		...
		doAjax: function (sUrl, sMethod, oData, rSuccess, rError, oHeaders) {
			...
			tempJsonModel.attachRequestFailed(function (oEvent) {
				that.oAppModel.setProperty("/requestSent", false);
				if (oEvent.getParameter("message") === "error") {
					that._showToastMessage(this.getView().getModel("i18n").getResourceBundle().getText("REFRESH_SCREEN_TEXT"));
				}
		...
		setNavigationDetailsFn: function () {
			...
			if (oSelectedView === "unifiedInbox") {
				...
				data = [{
						...
						"name": i18n.getText("USER_ROLE_MANAGEMENT_TEXT"),
						"inboxId": "userRoleManagement"
					}, {
						//i18n.getText("MANAGE_DASHBOARD_TILES")
						"name": "Manage Tiles",
					...
					},*/
					{
						...
						"inboxId": "advancedWorkFlow"
							// to make this work remove the enabled property in App.view -tnt:NavigationListItem
							/*"dtoList": [{
							    "name": i18n.getText("CREATE_WORKFLOW_TEXT"),
		...
		getLoggedInUserDetails: function () {
			var oAppModel = this.oAppModel;
			var sUrl = "/oneappincturereportreports/WorkboxJavaService/idpMapping/getUsersDetails";
		...
		setInboxPanel: function () {
			this.doAjax("/oneappincturereportreports/WorkboxJavaService/inbox/inboxPanel", "GET", null, function (oData) {
		...
		getStandardSettings: function () {
			var url = "/oneappincturereportreports/WorkboxJavaService/notificationConfiguration/getAllSettingDetails"
		...
		updateNotification: function () {
			var url = "/oneappincturereportreports/WorkboxJavaService/notification/getNotification";
			var oUserSettingsModel = this.getModel("oUserSettingsModel");
			...
			this.socket.onmessage = function (event) {
				...
				/* ------------------ For Collaboration Section -----------------*/
					...
					if (sChatId === oReceived.chatID) {
						...
						if (oReceived.messageType !== "acknowledge" && oReceived.messageType !== "typing") {
		...
		downloadAttachment: function (attachmentId, fileType, fileName, base64File, fileSize, bDownloadAll, encodedFile, isChat) {
			...
			if (attachmentId) {
				if (isChat) {
					...
					var url = "/oneappincturereportreports/WorkboxJavaService/detailPage/getOriginalAttachment/" + attachmentId;
					this.doAjax(url, "GET", null, function (oData) {
		...
		navigateToTaskDetail: function (oNotification) {
			var url = "/oneappincturereportreports/WorkboxJavaService/inbox/filterService";
			oNotification.notificationType = oNotification.notificationType.toLowerCase();

			var filterData;

			if (oNotification.notificationType === "new task assigned" || oNotification.notificationType === "forward") {
				filterData = {
					"quickFilter": {
			...
			} else if (oNotification.notificationType.includes("complete") || oNotification.notificationType.includes("resolve") ||
				oNotification.notificationType.includes("approve") || oNotification.notificationType.includes("reject") ||
				oNotification.notificationType === "done") {
				filterData = {
					"quickFilter": {},
		...
		fetchSavedFilters: function () {
			...
			var url = "/oneappincturereportreports/WorkboxJavaService/userCustomFilter/getFilters";
			this.doAjax(url, "GET", null, function (oData) {
