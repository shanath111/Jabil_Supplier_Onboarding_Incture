sap.ui.define([
	...
	return BaseController.extend("oneapp.incture.processFlow.processFlow.controller.App", {
		formatter: formatter,
		utility: utility,
		...
		getCommonServicesFn: function () {
			...
			this.getProcessList("All");
		},
		getUserList: function (rSuccess) {
			var oConstantsModel = this.getModel("oConstantsModel");

			var iurl = "/oneappinctureprocessFlowprocessFlow/WorkboxJavaService/idpMapping/getUsers";
		...
		getProcessList: function (processType, rSuccess) {
			var url = "/WorkboxJavaService/customProcess/getProcess?processType=" + processType;
			var oConstantsModel = this.getModel("oConstantsModel");
		...
		onSelectApp: function (oEvent) {
			...
			if ((oAppModel.getProperty("/currentView")) === "adminConsole") {
				...
				if (selectedKey === "graphConfiguration") {
		...
		setSearchFilterDto: function () {
			...
			if (!oAdvanceFilterModel.getProperty("/standardFilter") || oAdvanceFilterModel.getProperty("/standardFilter").length === 0) {
				var url = "/WorkboxJavaService/inbox/getMetadata";
				this.doAjax(url, "GET", null, function (oData) {
					var oCheckBox = {
						"filterType": "CHECKBOX",
						"dropdownList": []
		...
		remodelMetaData: function (oData, bindingObject, index, aIndex, type) {
			...
			if (bindingObject.columnName === "DESCRIPTION") {
				oAdvanceFilterModel.setProperty("/subjectIndex", "/standardFilter/" + index);
			}
			if (bindingObject.columnName === "NAME") { // not required
				...
				this.aggregateDropdownList("/WorkboxJavaService" + bindingObject.selectionList, index, aIndex, true);
			} else if (bindingObject.conditionList && (bindingObject.filterType === "DATEFILTER" || bindingObject.filterType === "COMBOINPUT")) {
				this.aggregateDropdownList("/WorkboxJavaService" + bindingObject.conditionList, index, aIndex, true);
		...
		// Get custom attribute based on process name and push to standard filter  // not required
		getCustomAttributes: function (processName, rSuccess) {
			...
			var url = "/WorkboxJavaService/inbox/getMetadata?processName=" + processName;
			if (processName !== "") {
				this.doAjax(url, "GET", null, function (oData) {
		...
		removeFilterTokenFn: function (oEvent) {
			...
			if (tokenDto[i].type === "quickFilter") {
				...
				if (tokenDto[i].sIndex && tokenDto[i].aIndex) {
					if (!isDuplicate || tokenDto[i].key === "DESCRIPTION") {
						oStandardFilter.value = "";
						oStandardFilter.selectedKey = "";
				...
				if (tokenDto[i].filterType === "MULTICOMBOBOX") {
					if (tokenDto[i].key === "NAME") { // not required
		...
		onChangeGlobalSearch: function (oEvent) { // not required
		...
		generateSearchFilters: function () {
			...
			advancedFiltersContent.addItem(advancedFilterHeader); // not required
		...
		aggregateAdvancedFilters: function (path, bindingObject) { // not required
		...
		setOperatorAdvFilter: function (rowIndex) { // not required
			var oAdvanceFilterModel = this.getModel("oAdvanceFilterModel");
		...
		onChangeMultiCombo: function (oEvent) {
			...
			if (prevSelectedKey.length === 1 && prevSelectedKey.length === selectedKey.length && selectedKey[0] === prevSelectedKey[0]) {
				if (!selectedKey[0]) {
					contextObject.selectedKey = [];
					oEvent.getSource().removeSelectedKeys();
				}
				return;
			}
		...
		suggestAutoComplete: function (oEvent, isSuggest) {
			...
			if (value) {
				...
				var sUrl = "/WorkboxJavaService" + oContext.selectionList + value;
		...
		setSelectedFiltersSearchFilter: function (oEvent) { // not required
		...
		removeSelProcess: function (removePN, removeCA) { // not required
			var oAdvanceFilterModel = this.getModel("oAdvanceFilterModel");
			this.getCustomAttributes("");
			if (removePN) {
		...
		setFilterNameDropdownInFilter: function (checkCA) { // not required
			var oAdvanceFilterModel = this.getModel("oAdvanceFilterModel");
			var aAdvancedFilterNames = oAdvanceFilterModel.getProperty("/aAdvancedFilterNames");
			var selectedFilterNames = oAdvanceFilterModel.getProperty("/selectedFilterNames");
			var listOfAdvancedFilters = oAdvanceFilterModel.getProperty("/listOfAdvancedFilters");
		...
		checkRowComplete: function (rowIndex) { // not required
			var oAdvanceFilterModel = this.getModel("oAdvanceFilterModel");
		...
		generateQuery: function (row, boolean) { // not required
			var oAdvanceFilterModel = this.getModel("oAdvanceFilterModel");
			var listOfAdvancedFilters = oAdvanceFilterModel.getProperty("/listOfAdvancedFilters");
			var queryList = oAdvanceFilterModel.getProperty("/queryList");
			var text = "";
		...
		clearFilterFn: function () {
			var oAdvanceFilterModel = this.getModel("oAdvanceFilterModel");
			if (oAdvanceFilterModel.getProperty("/showAdvancedFilter")) {
				this.clearBasicFilterData(); //only this
			} else {
				this.clearAdvFilterData();
				oAdvanceFilterModel.setProperty("/selectedFilterNames", []);
			}
		...
		addRowSearchFilter: function (oEvent) { // not required
			var oAdvanceFilterModel = this.getModel("oAdvanceFilterModel");
		...
		applyFilter: function (oEvent) {
			if (this.oAppModel.getProperty("/currentViewPage") === 'chat') { // not required
				/***************** Collaboration changes - BY Karishma *********************/
				var value = oEvent.getParameter("value").replace(/^\s+/g, "");
				...
				} else { //Apply advanced filter //only this
		...
		applyAdvancedFilter: function (requestPayload) {
            ...
            router.navTo("UnifiedInbox", {}, true);
				...
				// When apply button is clicked from any view, handle the inbox type
				if (oAppModel.getProperty("/currentView") === "dashBoard" || oAppModel.getProperty("/currentView") === "processFlow" &&
					oAppModel.getProperty("/currentView") === "taskDetailPage") {
					var searchInboxType = this.getModel("oAdvanceFilterModel").getProperty("/searchInboxType");
					var inboxName = this.byId("ID_GLOBAL_SEARCH_INBOX_TYPE").getSelectedItem().getText();
					this.setCurrentPage(searchInboxType, searchInboxType, inboxName, true, true);
					this._doNavigate("UnifiedInbox", {});
				} else if (oAppModel.getProperty("/currentView") === "unifiedInbox") {
					if (oAppModel.getProperty("/WB_TASK_BOARD_VIEW")) {
						sap.ui.core.UIComponent.getRouterFor(this).getView('oneapp.incture.workbox.view.UnifiedInbox').getController().getTaskBoardData();
		...
		validateAdvFilter: function () {
			...
			if (this._oAdvanceFilter !== undefined) {
				...
				if (this._oAdvanceFilter.isOpen() && (oAdvanceFilterModel.getProperty("/showAdvancedFilter") === true)) {
					...
					savedFilter.filterType = "BASIC"; // not required
				}
				// advanced filters
			...
			var response = {
				...
				"saveFilterPayload": savedFilter // not required
			};
		...
		changeWorkdayPayload: function (filterMap) { // not required
		...
		generateTokens: function (oStandardFilter, level, advFilterType) {
			...
			if (oStandardFilter.columnName === "DESCRIPTION") {
		...
		validateFilterName: function (oEvent) { // not required
		...
		closeSaveFilter: function (oEvent) { // not required
		...
		applySaveFilter: function (property, oEvent) { // not required
			var path = "";
			var url = "/WorkboxJavaService/userCustomFilter/saveViewFilter";
			var flag = true,
				isTile = false;
			var oAdvanceFilterModel = this.getModel("oAdvanceFilterModel");
			var filterData;
			...
			if (property === "CreateFilter" || property === "CreateTile") {
				...
				if (property === "DeleteFilter" || property === "DeleteTile") {
					...
					property = "Delete";
					url = "/WorkboxJavaService/userCustomFilter/deleteFilter";
					var delFlag = flag;
					flag = false;
					var i18n = this.getView().getModel("i18n").getResourceBundle();
		...
		confirmUpdateFilter: function (property, url, path, filterData, flag, isTile) { // not required
			var oAdvanceFilterModel = this.getModel("oAdvanceFilterModel"),
				that = this,
				payload = {},
		...
		editTileFilter: function (oEvent) { // not required
		...
		cloneTileFilter: function (oEvent) { // not required
			var oSelectedTile = oEvent.getSource().getBindingContext("oAdvanceFilterModel").getObject(),
			...
			this.confirmUpdateFilter("Create", "/WorkboxJavaService/userCustomFilter/saveViewFilter", "", oSelectedTile.filterData,
				true,
				true);
		},
		//on dropping the list item the sequence no and isActive is set
		onDropSavedTile: function (oInfo, sPath) { // not required
		...
		saveAllFilters: function (sPath) { // not required
			var oAdvanceFilterModel = this.getModel("oAdvanceFilterModel"),
				filters = oAdvanceFilterModel.getProperty(sPath);
			var url = "/WorkboxJavaService/userCustomFilter/saveAllViewFilter";
		...
		openManageFilter: function () {
			if (!this._oManageFilter) {
				this._oManageGraphContent = sap.ui.xmlfragment("oneapp.incture.workbox.fragment.GraphConfigurationView", this);
				this._oManageFilterContent = sap.ui.xmlfragment("oneapp.incture.workbox.fragment.ManageFilter", this);
			...
			if (this.getModel("oAppModel").getProperty("/currentView") === 'dashBoard') {
				...
				graphTabFiter.addContent(this._oManageGraphContent);
				var tileTabFiter = new sap.m.IconTabFilter({
					text: "{= ${oAppModel>/currentView} === 'dashBoard'? ${i18n>MANAGE_TILES_TEXT}:${i18n>MANAGE_FILTERS_TEXT}}",
					key: "manageTiles",
				});

				tileTabFiter.addContent(this._oManageFilterContent);
				oIconTabBarContent.addItem(graphTabFiter);
				oIconTabBarContent.addItem(tileTabFiter);
				this._oManageFilter.removeAllContent();
				this._oManageFilter.addContent(oIconTabBarContent);
				this.getUserGraphConfigDetails();
			} else {
				this._oManageFilter.removeAllContent();
				this._oManageFilter.addContent(this._oManageFilterContent);
			}
			this.fetchSavedFilters();
		...
		fetchGraphListfn: function (oDefaultDataModel, user) {
			var surl = "/WorkboxJavaService/Dashboard/getGraphList";
			if (user === "admin") {
		...
		fetchGraphParameterfn: function (oDefaultDataModel) {
			var surl = "/WorkboxJavaService/crossConstant/getConstants/graphParam";
			this.doAjax(surl, "GET", null, function (oData) {
					if (oData) {
		...
		changeFilterItemsFn: function (oEvent, oDefaultDataModel) {
			...
			} else if (value === "BUSINESS_STATUS" || value === "ACTION_STATUS") {
				var url = "/WorkboxJavaService/inbox/selectionList?selectionParameter=" + value;
				this.doAjax(url, "GET", null, function (oData) {
					oDefaultDataModel.setProperty("/graphConfigFilterData", oData.keyValuePairs);
					oDefaultDataModel.refresh(true);
				}.bind(this), function (oError) {}.bind(this));
			}
		...
		graphConfigPostFn: function (oDefaultDataModel, user) {
			...
			if (validationCheck) {
				...
				var url = "/WorkboxJavaService/Dashboard/createGraph";
				if (user === "admin") {
					url = url + "?type=Admin";
				}
		...
		deleteGraphConfigfn: function (oEvent, oDefaultDataModel, user) {
			...
			var url = "/WorkboxJavaService/Dashboard/deleteGraph/" + graphId;
			if (user === "admin") {
				url = url + "?type=Admin";
			}
			this.doAjax(url, "GET", null, function (oData) {
		...
		onGraphListItemSelect: function (oEvent, oDefaultDataModel, view) {
			...
			var url = "/WorkboxJavaService/Dashboard/getGraphDetails/" + graphId;
			this.doAjax(url, "GET", null, function (oData) {
		...
		activeGraphfn: function (oEvent, oDefaultDataModel) {
			...
			var url = "/WorkboxJavaService/Dashboard/updateGraph";
		...
		onSaveGraphListSequence: function (oDefaultDataModel) {
			...
			var url = "/WorkboxJavaService/Dashboard/updateGraphList";
			this.doAjax(url, "POST", data, function (oData) {
				this._showToastMessage(oData.message);
				this.oAppModel.setProperty("/isChanged", false);
				this.oAppModel.refresh(true);
		...
		onSelectSavedFilter: function (oEvent) {
			...
			this.setCurrentPage(oContext.inboxType, oContext.inboxType, oContext.filterName, true, false);
			oAdvanceFilterModel.setProperty("/viewAppliedContext", oContext);
		...
		bulkDeleteDraft: function () {
			...
			for (var i = 0; i < requestIds.length; i++) {
				var url = "/WorkboxJavaService/workbox/tasks/deleteDraft/" + requestIds[i].processId + "/" + requestIds[i].requestId;
				this.doAjax(url, "GET", null, function (oData) {
					this._showToastMessage(oData.message);
		...
		getNoticationChannels: function () {
			var url = "/WorkboxJavaService/customProcess/getDropDownValues/Notification%20Channels";
			this.doAjax(url, "GET", null, function (oData) {
		...
		saveGeneralSettings: function (oEvent) {
			...
			if (oEvent) {
				...
				this.doAjax("/WorkboxJavaService/idpMapping/updateUserDetail", "POST", payload, function (oData) {
					this._showToastMessage(oData.message);
		...
		loadSubstitutionData: function (property) {
			...
			if (property === "mySubs") {
				url = "/WorkboxJavaService/substitutionRule/substituting";
			}
			if (property === "iAmSubs") {
				url = "/WorkboxJavaService/substitutionRule/substituted";
			}
			if (property === "allSubs") {
				serviceType = "POST";
				payload = {};
				url = "/WorkboxJavaService/substitutionRule/getRules";
			}
			...
			if (property === "Discard") {
				...
				if (flag === true) {
					...
					if (property === "Create") {
						...
						url = "/WorkboxJavaService/substitutionRule/delete";
						var i18n = this.getView().getModel("i18n").getResourceBundle();
						var warning = i18n.getText("WARNING_TEXT");
						var alertmessage = i18n.getText("DELETE_CAUTION_TEXT");
						this._createConfirmationMessage(warning, alertmessage, "Warning", i18n.getText("YES_TEXT"), i18n.getText("NO_TEXT"), true,
		...
		isSubsEdited: function (oEvent, property) {
			...
			if (property) {
				if (property === "DateRange") {
					oUserSettingsModel.setProperty("/addSubstituteDto/startDate", oEvent.getParameter("from"));
					oUserSettingsModel.setProperty("/addSubstituteDto/endDate", oEvent.getParameter("to"));
		...
		substitutionITBSelect: function (oEvent) {
			...
			var oUserSettingsModel = this.getModel("oUserSettingsModel");
			oUserSettingsModel.setProperty("/selectedSubstitution", selectedKey);
		...
		removeNotification: function (count, id) {
			var url = "/WorkboxJavaService/notification/removeNotification/";
			if (count === "ALL") {
				url = url + "ALL";
			} else {
		...
		loadNotification: function () {
			var oUserSettingsModel = this.getModel("oUserSettingsModel");
			oUserSettingsModel.setProperty("/settings/busy", true);
			var url = "/WorkboxJavaService/notificationConfiguration/getViewList";
			this.doAjax(url, "GET", null, function (oData) {
		...
		loadProfileNotificationSettings: function () {
			var oUserSettingsModel = this.getModel("oUserSettingsModel");
			oUserSettingsModel.setProperty("/settings/busy", true);
			var url = "/WorkboxJavaService/notificationConfiguration/getProfiles";
		...
		getNotifEventsDetails: function (viewType, viewName) {
			...
			var url = "/WorkboxJavaService/notificationConfiguration/getViewDetail",
				payload = {
					"viewType": viewType,
					"viewName": viewName,
					// "userId": null
		...
		generateNotifSettings: function (settingsParams) {
			var oUserSettingsModel = this.getModel("oUserSettingsModel");
			var url = "/WorkboxJavaService/notificationConfiguration/getSettingDetails?" + settingsParams;
		...
		addNotificationDropdownList: function (bindingObject) {
			...
			this.doAjax("/WorkboxJavaService" + bindingObject.selectionList, "GET", null, function (oData) {
		...
		updateGeneralSettings: function (oEvent, action) {
			...
			if (action === "UPDATE") {
				...
				url = "/WorkboxJavaService/notificationConfiguration/updateViewList";
			} else {
				...
				if (configType === "ADDGROUP") {
					...
					url = "/WorkboxJavaService/notificationConfiguration/createWhatObject"
				} else {
					oPayload = {
						"viewDtos": [{
							"viewName": oNewConfiguration.viewName,
					...
					url = "/WorkboxJavaService/notificationConfiguration/createChannel";
				}
			}
			this.doAjax(url, "POST", oPayload, function (oData) {
				this._showToastMessage(oData.message);
		...
		saveNotificationSettings: function () {
			...
			if (oUserSettingsModel.getProperty("/settings/notification/newEventAdded")) {
				url = "/WorkboxJavaService/notificationConfiguration/createEvents";
				var oSelectedObj = oUserSettingsModel.getProperty("/settings/selectedNotification")
				oPayload = {
					"eventDtos": []
				};
			...
			if (oUserSettingsModel.getProperty("/settings/notification/eventsDeleted")) {
				...
				url = "/WorkboxJavaService/notificationConfiguration/deleteViewDetailByAdmin";
				this.doAjax(url, "POST", oPayload, function (oData) {
					// this._showToastMessage(oData.message);
					var viewType, viewName;
					var oSelectedObj = oUserSettingsModel.getProperty("/settings/selectedNotification")
			...
			if (oUserSettingsModel.getProperty("/settings/notification/notificationEventChanged")) {
				url = "/WorkboxJavaService/notificationConfiguration/updateViewDetail";
				var oSelectedObj = oUserSettingsModel.getProperty("/settings/selectedNotification")
				oPayload = {
			...
			if (oUserSettingsModel.getProperty("/settings/notification/notificationSettingsChanged")) {
				url = "/WorkboxJavaService/notificationConfiguration/updateSettings";
				if (oUserSettingsModel.getProperty("/settings/selectedSetting") === "SETTINGS_ADMIN_NOTIFICATIONS") {
					url = url + "ByAdmin";
				}
		...
		deleteNotificationConfig: function (oEvent, type) {
			...
			if (type === "EVENTLIST") {
				...
				url = "/WorkboxJavaService/notificationConfiguration/deleteViewList";
				payload.viewName = oSelectedObject.viewName;
				if (type === "CHANNEL") {
					url = "/WorkboxJavaService/notificationConfiguration/deleteChannel";
				}
				var i18n = this.getView().getModel("i18n").getResourceBundle();
				var warning = i18n.getText("WARNING_TEXT");
				var alertmessage = i18n.getText("DELETE_CAUTION_TEXT");
		...
		updateProfile: function (oEvent, action) {
			...
			if (action === "CREATE") {
				...
				url = "/WorkboxJavaService/notificationConfiguration/createProfile";
				if (oUserSettingsModel.getProperty("/settings/selectedSetting") === "SETTINGS_ADMIN_NOTIFICATIONS") {
					url = url + "ByAdmin";
				}
				...
				url = "/WorkboxJavaService/notificationConfiguration/deleteProfile";
				if (oUserSettingsModel.getProperty("/settings/selectedSetting") === "SETTINGS_ADMIN_NOTIFICATIONS") {
					url = url + "ByAdmin";
				}
			} else if (action === "SAVE") {
				url = "/WorkboxJavaService/notificationConfiguration/updateProfile";
				if (oUserSettingsModel.getProperty("/settings/selectedSetting") === "SETTINGS_ADMIN_NOTIFICATIONS") {
					url = url + "ByAdmin";
		...
		getAllVersions: function () {
			this.doAjax("/WorkboxJavaService/versionControl/getAllVersion", "GET", null, function (oData) {
				// oData.versions = []
				var oUserSettingsModel = this.getModel("oUserSettingsModel");
		...
		loadVersions: function (versionNumber) {
			var oUserSettingsModel = this.getModel("oUserSettingsModel");
			oUserSettingsModel.setProperty("/settings/busy", true);
			var url = "/WorkboxJavaService/versionControl/getVersionDetail?versionNumber" + versionNumber;
			this.doAjax(url, "GET", null, function (oData) {
		...
		fnUploadAttachment: function (oFile, versionId) {
			...
			var sAttachmentUrl = "/WorkboxJavaService/versionControl/upload";
			setTimeout(function () {
		...
		openLinkVersionControl: function (documentId, link) {
			...
			if (link) {
				...
				var url = "/WorkboxJavaService/versionControl/getDocumentByID?documentID=" + documentId;
			...
			if (flag) {
				...
				if (newVersion.whatsNew.length > 0 || newVersion.bugFixes.length > 0 || newVersion.improvements.length > 0) {
					...
					if (inValidLink) {
						...
						var url = "/WorkboxJavaService/versionControl/createNewVersion";
		...
		showTaskDetails: function (oEvent) {
			...
			if (origin === "Task") {
				if (oNotification.notificationType === "Chat") {
					var oCollaborationModel = this.getOwnerComponent().getModel("oCollaborationModel");
					this.getView().setModel(oCollaborationModel, "oCollaborationModel");
					oCollaborationModel.setProperty("/oChatNotification", oNotification);
					oCollaborationModel.setProperty("/bFromNotification", true);
					this.fnNavigateToChatPage(oNotification);
				} else {
		...
		fnNavigateToChatPage: function (oNotification) {
		...
		sendEmailfn: function () {
			...
			var url = "/WorkboxJavaService/emailTemplate/saveEmailTemplate";
			this.doAjax(url, "POST", data, function (oData) {
				this._showToastMessage(oData.message);
		...
		onImportSelectedRow: function (oEvent) {
			...
			var oCustomTaskModel = this.getModel("oCustomTaskModel");
			taskManagement.onImportSelectedRowTM(oCustomTaskModel, this, ExtDatePicker);
		},
